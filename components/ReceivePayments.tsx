...
export default function ReceivePayments({ userAddress }: ReceivePaymentsProps) {
    ...
    try {
      ...
      if (data.success) {
        ...
        const receivedPayments = data.payments
          .filter(
            (payment: any) =>
              payment.receiverAddress.toLowerCase() ===
          ...
          .map((payment: any) => {
            ...
            const isExpired =
              ...
              now > payment.expirationTimestamp;
  ...
  const WithdrawMoney = async (sender: string) => {
    // Placeholder function if needed in future
    const transaction: InputTransactionData = {
      data: {
        function: `${CONFIG.MODULE_ADDRESS}::escrow_vault::withdraw`,
  ...
  const handleClaimPayment = async (paymentId: string) => {
    ...
    try {
      ...
      const txHash = await WithdrawMoney(payment.senderAddress);
      ...
      toast.success(
        `Payment claimed successfully! Transaction: ${result.hash}`
      );
    } catch (error) {
      ...
      <div className="pt-12">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex space-x-2">
            {(["all", "pending", "completed", "expired"] as const).map(
              (status) => (
                <button
                  key={status}
              ...
              >
                <div className="flex items-center justify-between">
                    ...
                    <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center">
                      <span className="text-white font-bold">
                        {payment.senderName === "Unknown Sender"
                          ? "?"
                          : payment.senderName.charAt(0).toUpperCase()}
                      </span>
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    ...
                    {payment.status === "pending" &&
                      payment.expiryTimestamp && (
                        <div className="mt-2">
                          <p className="text-xs text-slate-400">
                            Expires in:{" "}
                            {getTimeUntilExpiry(
                              parseInt(payment.expiryTimestamp)
                            )}
                          </p>
                        </div>
                      )}
